...
namespace calcu_field{
    int byft::calcu_pws(std::string title){
        ...
        std::cout << "near_ref.Exyz.cols = " << near_ref.Exyz.cols() <<std::endl;
        A_pws = A_pws / near_ref.Exyz.cols() / (2*pai);
        std::cout << "A_pws = \n" << A_pws << std::endl;

        int sample_oneaxis = std::sqrt(fardata.n_sample);
        MatrixXd vec_kz = Matrix<double,1,Dynamic>::Zero(1,sample_oneaxis);
        for(int i = 0 ; i < sample_oneaxis ; i++){
            vec_kz(0,i) = k * cos(fardata.Rpolar(1,i));
        }

        // std::cout << "sample_oneaxis = " << sample_oneaxis << std::endl;
        // for(int row = 0 ; row < sample_oneaxis ; row++){
        //     for(int col = 0 ; col < sample_oneaxis ; col++){
        //         fardata.Exyz(0,row*sample_oneaxis+col) = A_pws(0,row*sample_oneaxis+col) * vec_kz(row) * vec_kz(col);
        //         std::cout << row*sample_oneaxis+col << " ";
        //     }
        // }
        // std::cout << std::endl;
        ...
        // data_field::make_graph_xcut(near_ref,near_ref,title);
        // data_field::make_graph_ycut(near_ref,near_ref,title);

        // plot_field_global(val_x,power_db,key_info,graph_info);
        data_field::make_graph_xcut(fardata,far_ref,title);
        data_field::make_graph_ycut(fardata,far_ref,title);
        return 0;
    }
}